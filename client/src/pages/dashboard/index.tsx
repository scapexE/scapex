...
export default function Dashboard() {
  ...
  // Step 1: filter by activity modules if an activity is active
  const activityModules = activeActivity?.modules ?? null;

  // Step 2: filter by user permissions (admin sees all)
  const visibleApps = ALL_APPS.filter((app) => {
    if (app.id === "system_admin") return false; // never shown in app grid
    if (isAdmin) {
      // admin: only filter by active activity modules if one is selected
      return activityModules ? activityModules.includes(app.id) : true;
    }
    // non-admin: must have permission AND activity must include module
