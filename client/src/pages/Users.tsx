...
export default function Users() {
      ...
      <MainLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">فقط مدير النظام يمكنه إدارة المستخدمين</p>
          </div>
        </div>
      </MainLayout>
    );
  ...
  const handleRoleChange = (role: Role, form: Partial<SystemUser>, setForm: (f: Partial<SystemUser>) => void) => {
  ...
  const handleAddSubmit = () => {
    ...
    if (users.find((u) => u.email === form.email)) {
      toast({ title: "خطأ", description: "البريد الإلكتروني مستخدم مسبقاً", variant: "destructive" });
      return;
    }
    const newUser: SystemUser = {
      ...
      permissions: form.permissions || ROLE_DEFAULTS[form.role as Role],
  ...
  const togglePerm = (modId: string) => {
    ...
    setPermUser({
      ...permUser,
      permissions: has ? permUser.permissions.filter((p) => p !== modId) : [...permUser.permissions, modId],
    });
  };
                    ...
                    {mods.map((mod) => {
                      ...
                      return (
                        ...
                        >
                          <Checkbox
                            checked={checked}
                            onCheckedChange={() => togglePerm(mod.id)}
...
function UserForm({
  ...
  setForm: (f: Partial<SystemUser>) => void;
  showPass: boolean;
    ...
    <div className="space-y-4" dir="rtl">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          سيتم ضبط الصلاحيات تلقائياً حسب الدور. يمكنك تعديلها لاحقاً.
        </p>
      </div>
